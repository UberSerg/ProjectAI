...
import { ExternalArchivePanel } from "../features/market/ExternalArchivePanel";
...
type MarketTab = "instruments" | "archive";
...
export function MarketPage() {
  ...
  const [tab, setTab] = useState<MarketTab>("instruments");
  ...
  useEffect(() => {
    ...
    return () => controller.abort();
  }, [search, assetClass, source, active, page, tab]);
  ...
  return (
    <section>
      ...
      <div className="tabs" role="tablist">
        <button
          type="button"
          className={`tab${tab === "instruments" ? " active" : ""}`}
      ...
      {tab === "archive" ? <ExternalArchivePanel /> : null}
        ...
        <>
          ...
          {error ? (
            <PageState
              kind="error"
              action={
                <button type="button" onClick={() => setPage((p) => p)}>
