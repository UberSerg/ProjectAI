...
@patch("app.modules.investment.application.services._schema_ready", return_value=True)
@patch("app.modules.investment.application.services.list_bonds")
def test_get_bond_detail_none(mock_list, _schema) -> None:
...
@patch("app.modules.investment.application.services._schema_ready", return_value=True)
@patch("app.modules.investment.application.services.list_bonds")
def test_get_bond_detail_with_cashflows(mock_list, _schema) -> None:
    mock_list.return_value = [
        {
            "instrument_id": 1,
            "symbol": "SU26230RMFS1",
    ...
    cf.source = "MOEX"
    session = MagicMock()
    session.scalar.return_value = SimpleNamespace(id=1, symbol="SU26230RMFS1", name="OFZ")
